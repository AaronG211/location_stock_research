...
\noindent \textbf{Keywords:} geographic co-movement, stock returns,
headquarters location, location factor, Pirinsky--Wang replication, MSA,
Fama--MacBeth
...
identify an additional dimension of return co-movement: \emph{geographic
location}. Their study shows that firms headquartered in the same
...
Their explanation centers on \emph{local investor sentiment}. Because
retail and institutional investors tend to choose to invest in local
stocks---a phenomenon commonly referred to as the home bias---correlated
trading by geographically concentrated investors induces co-movement
among firms in the same area. Importantly, this co-movement is not
driven by local economic fundamentals; Pirinsky and Wang (2006) show
that their results survive controls for local macroeconomic variables.
...
sub-period of 2008--2022. Second, the S\&P 500 results show that the
conclusion for large-cap firms depends on how the location and industry
benchmarks are constructed: the location effect is not distinguishable
from zero when those benchmarks are equally weighted, but it becomes
positive and statistically significant when they are weighted by market
capitalization. The joint pattern suggests persistence rather than
disappearance: the modern location factor remains economically
meaningful, but its visibility depends on how the location and industry
...
findings and their implications. Section 7 concludes.
...
Fama-French Industries & SIC code to Fama-French 48 industry classification mapping & Static reference & Industry classification for benchmark construction\\
\bottomrule
\end{tabular}}
\end{table}
...
database provides the Central Index Key (\(CIK\)), which is the SEC's
unique firm identifier used in 10-K filings. The CCM linktable bridges
the Compustat identifier (\(GVKEY\)) to the CRSP identifier
(\(PERMNO\)), enabling a three-way mapping:
\(CIK \leftrightarrow GVKEY \leftrightarrow PERMNO\). Primary link
records (link type LC or LU) are retained and aligned to filing dates
during preprocessing.

\textbf{Loughran-McDonald 10-K Headers.} The Loughran-McDonald Master
Dictionary dataset includes parsed header information from all SEC 10-K
filings from 1993 to 2024. The geographic fields are extracted: business
address state, city, and ZIP code, as well as mailing address
equivalents. Each filing is identified by \(CIK\) and filing date. This
dataset serves as the primary source for determining the physical
location of each firm's headquarters, which is a critical distinction
from using state of incorporation (a legal, rather than economic,
concept).

\textbf{HUD ZIP-CBSA Crosswalk.} The U.S. Department of Housing and
Urban Development (HUD) provides a quarterly crosswalk mapping
five-digit ZIP codes to Core-Based Statistical Areas (CBSAs), which
correspond to Metropolitan Statistical Areas (MSAs). When a ZIP code
maps to multiple CBSAs, the CBSA with the highest business activity
ratio (\(BUS\_RATIO\)), defined by HUD as the share of business
addresses in that ZIP assigned to a given CBSA, is selected. Rural ZIP
codes (HUD code 99999) are excluded.

\textbf{Fama-French 48 Industries.} The Fama-French 48-industry
classification maps four-digit SIC codes to 48 industry groups. This
classification is standard in empirical asset pricing and provides a
consistent industry taxonomy across the full sample period.
...
the return regressions and benchmark construction so that unmatched
firms are not dropped mechanically, but it is excluded from the
...
plurality of commercial activity at that ZIP code occurs. Rural areas
(HUD code 99999) are coded as missing, as they lack the agglomeration
effects central to the Pirinsky--Wang hypothesis.
...
industries in that month. Location benchmarks constructed from less data
than this threshold would be excessively noisy and vulnerable to
single-industry shocks.
...
stock files that survive all quality screens. Table 2 reports the key
sample sizes used in the empirical section.

\begin{table}[!h]
\centering
\caption{\label{tab:sample-attrition}Key Sample Sizes in the Final Pipeline}
\centering
\fontsize{10}{12}\selectfont
\begin{tabular}[t]{>{\raggedright\arraybackslash}p{11cm}>{\raggedright\arraybackslash}p{2.5cm}}
\toprule
Stage & N\\
\midrule
Post-cleanup research sample (valid CSV files) & 5,088\\
Valid firm-level regressions, 2008--2012 (all stocks) & 3,275\\
Valid firm-level regressions, 2013--2017 (all stocks) & 3,158\\
Valid firm-level regressions, 2018--2022 (all stocks) & 3,146\\
Valid firm-level regressions, S\&P 500 subsample (by window) & 382 / 394 / 377\\
\bottomrule
\end{tabular}
\end{table}
...
bucket). Note that due to the 24-month temporal continuity filter
applied after benchmark construction, some MSAs in the surviving
estimation sample may display fewer than the initial five-firm threshold
in these snapshot months.

\begin{table}[!h]
\centering
\caption{\label{tab:table1-panel-a}Summary Statistics Panel A: Firms per MSA (Snapshot Months)}
\centering
\fontsize{10}{12}\selectfont
...
\begin{table}[!h]
\centering
\caption{\label{tab:table1-panel-b}Summary Statistics Panel B: Industries per MSA (Snapshot Months)}
\centering
\fontsize{10}{12}\selectfont
...
Following Pirinsky and Wang (2006), three benchmark return series are
constructed for each stock \(i\) in month \(t\): a location benchmark,
an industry benchmark, and a market benchmark. Industry and location
benchmarks are constructed using a \emph{leave-one-out} methodology,
excluding the current stock when calculating its own benchmarks so as to
prevent mechanical correlation between the dependent variable and the
regressors.

\subsubsection{Location Benchmarks}\label{location-benchmarks}

Let \(L_i\) denote the MSA of firm \(i\)'s headquarters, and let
\(\mathcal{N}_{L_i,t}\) denote the set of all firms headquartered in MSA
\(L_i\) in month \(t\), excluding firm \(i\) itself. Two location
benchmarks are used:
...
firm \(i\)'s own return does not enter its location benchmark, which
would otherwise bias \(\hat{\beta}_{Loc}\) upward. This distinction
matters for interpretation: Model 2 changes not only the industry factor
but also the location factor, replacing the equal-weighted local-peer
benchmark with a value-weighted local-peer benchmark.

\subsubsection{Industry Benchmarks}\label{industry-benchmarks}

Let \(I_i\) denote the Fama-French 48-industry classification of firm
\(i\), and let \(\mathcal{N}_{I_i,t}\) denote the set of all firms in
industry \(I_i\) in month \(t\), excluding firm \(i\). Two industry
benchmarks are constructed:
...
\noindent The equal-weighted benchmark treats all industry peers
symmetrically, while the value-weighted benchmark assigns greater
influence to large-cap firms. If the location effect were merely a proxy
for the movements of dominant firms within an industry (e.g., Apple in
...
\subsubsection{Market Benchmark}\label{market-benchmark}

The market benchmark is the value-weighted return of all stocks in the
sample:
...
month \(t\). Unlike the location and industry benchmarks, the market
benchmark is not leave-one-out, as the influence of any single firm on
the aggregate market return is negligible.

\subsection{Regression Specification}\label{regression-specification}

For each stock \(i\), the following time-series regression is estimated
over each five-year sub-period \{2008--2012, 2013--2017, 2018--2022\}:
...
Benchmarks}\label{model-1-equal-weighted-industry-and-location-benchmarks}

Table 5 combines the main regression results for both specifications and
...
Figure 1 illustrates the location-benchmark composition for the most
...
considerably once both the industry and location benchmarks are
value-weighted, as shown in the next section.

\subsection{Model 2: Value-Weighted Industry and Location
Benchmarks}\label{model-2-value-weighted-industry-and-location-benchmarks}
...
statistically significant after both the location and industry
benchmarks are tilted toward larger firms. Model 2 therefore strengthens
the persistence result: even when the local benchmark itself is rebuilt
as a value-weighted portfolio, the geographic factor continues to
survive.
...
times larger. Second, while the original study reported a
\textit{declining} location effect over time (0.545 \(\to\) 0.532
\(\to\) 0.459), the modern broad-sample estimates move in the opposite
direction, tracing a monotone \textit{increase} (0.200 \(\to\) 0.213
\(\to\) 0.295). The decline during the late 20th century may have
reflected the gradual dissolution of local information asymmetries under
improving telecommunications, whereas the post-financial-crisis years
appear to have reversed that trend for the typical firm in the
cross-section. The S\&P 500 subsample adds a modern twist: the
equal-weighted specification yields little evidence of a large-cap
location effect, but the value-weighted specification restores a
positive and statistically significant loading, especially in the later
windows.
...
The main message of the modern sample is straightforward: the location
effect documented by Pirinsky and Wang (2006) has not disappeared. In
the broad sample, the location loading remains positive and
statistically significant in every five-year window under both
specifications. Under Model 1, \(\hat{\beta}_{Loc}\) rises from 0.200
(\(t=9.91\)) in 2008--2012 to 0.295 (\(t=13.33\)) in 2018--2022. Under
Model 2, where both the industry factor and the location factor are
value-weighted, the location loading is again positive and statistically
significant in all three windows, rising from 0.116 (\(t=4.93\)) to
0.222 (\(t=7.94\)).
...
S\&P 500 firms, but once the location and industry benchmarks are
...
statistically indistinguishable from zero under the equal-weighted
benchmark design, yet clearly positive under the value-weighted
benchmark design. Two observations help to rationalize this contrast.
First, an equal-weighted average of local peers is mechanically
dominated by smaller firms in the same MSA, whose co-movement with a
mega-cap anchor is modest. Second, once the location and industry
benchmarks are value-weighted, the dominant-firm component of return
variation is absorbed more cleanly, and whatever local co-movement
remains among the largest stocks is re-exposed rather than hidden inside
the regressors. The contrast between the two specifications is therefore
informative: it tells us that the location effect among large caps is
real but comparatively small, and that it can only be detected once the
benchmark construction no longer conflates peer structure with size.
...
\item
  ...
  and industry benchmarks are constructed in a size-aware way.
\item
...
from 2008 to 2022, a period that encompasses the global financial
crisis, the post-crisis recovery, the rise of passive investing, and the
COVID-19 pandemic. The results confirm that the geographic co-movement
effect persists in the modern era under the original equal-weighted
specification, albeit at roughly half of the 1990s magnitude in the
broad sample, and they show that the effect also survives a
specification in which both the industry benchmark and the location
benchmark are value-weighted. In the broad sample the location beta
...
These findings suggest that the digital revolution and the trend toward
remote work have not dissolved the geographic co-movement effect.
Physical corporate headquarters location continues to explain a
statistically significant portion of return co-movement in the modern
U.S. market, and that conclusion survives even when the location and
industry benchmarks are reweighted toward the largest firms. Future
