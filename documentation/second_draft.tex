...
\noindent \textbf{Keywords:} geographic return co-movement, stock
returns, headquarters location, Pirinsky--Wang replication, MSA,
...
identify an additional dimension of return co-movement: geographic
...
Their explanation centers on local investor sentiment. Because retail
...
sub-period of 2008--2022, but its magnitude has shrunk substantially
...
Second, the S\&P 500 results show that the conclusion for large-cap
firms depends on how the location and industry indexes are constructed:
the location effect is not distinguishable from zero when those indexes
are equally weighted, but it becomes positive and statistically
...
depends on how the location and industry indexes are constructed.
...
at \url{https://wrds-www.wharton.upenn.edu/} (accessed November 2025),
...
database, also accessed through WRDS at
\url{https://wrds-www.wharton.upenn.edu/} (accessed November 2025),
provides the Central Index Key (\(CIK\)), which is the SEC's unique firm
...
\textbf{Loughran-McDonald 10-K Headers.} The Loughran-McDonald 10-K
Header Data file, distributed through the Software Repository for
Accounting and Finance (SRAF) at
\url{https://sraf.nd.edu/sec-edgar-data/} (accessed November 2025),
includes parsed header information from all SEC 10-K filings from 1993
...
incorporation (a legal, rather than economic, concept). \textbf{HUD
...
Metropolitan Statistical Areas (MSAs). The crosswalk file used in this
study is the December 2023 vintage, downloaded from
\url{https://www.huduser.gov/portal/datasets/usps_crosswalk.html}
(accessed November 2025). When a ZIP code maps to multiple CBSAs, the
...
classification, originally proposed in Fama and French (1997), maps
four-digit SIC codes to 48 industry groups. The classification file is
obtained from Kenneth R.~French's online Data Library at
\url{https://mba.tuck.dartmouth.edu/pages/faculty/ken.french/data_library.html}
(accessed November 2025). This classification is standard in empirical
...
the return regressions and index construction so that unmatched firms
...
plurality of commercial activity at that ZIP code occurs. In practice,
...
6.6\% (303 ZIPs) straddle two or more. Rural areas (HUD code 99999) are
...
industries in that month. Location indexes constructed from less data
than this threshold would be excessively noisy and vulnerable to
single-industry shocks. The raw monthly panel spans 449 distinct MSAs,
...
firm-level regression sample sizes in each estimation window, separately
for the broad sample and the S\&P 500 subsample.
...
bucket). The snapshot counts (84, 82, and 81 MSAs) are lower than the
...
specific snapshot month. Additionally, the 24-month temporal continuity
filter applied after index construction may further reduce the number of
MSAs appearing in a given snapshot month.
...
Following Pirinsky and Wang (2006), three return series are constructed
for each stock \(i\) in month \(t\): a location index, an industry
index, and a market portfolio. Industry and location indexes are
constructed using a leave-one-out methodology, excluding the current
stock when calculating its own indexes so as to prevent mechanical
correlation between the dependent variable and the regressors.
Importantly, indexes are constructed from the cleaned monthly
...
\subsubsection{Location Indexes}\label{location-indexes}

Let \(\mathcal{N}_{i,t}\) denote the set of all firms sharing the same
MSA as firm \(i\) in month \(t\), excluding firm \(i\) itself. Two
location indexes are used:
...
firm \(i\)'s own return does not enter its location index, which would
...
also the location factor, replacing the equal-weighted local-peer index
with a value-weighted local-peer index.

\subsubsection{Industry Indexes}\label{industry-indexes}

Let \(\mathcal{M}_{i,t}\) denote the set of all firms sharing the same
Fama-French 48-industry classification as firm \(i\) in month \(t\),
excluding firm \(i\) itself. Two industry indexes are constructed:
...
\noindent The equal-weighted index treats all industry peers
symmetrically, while the value-weighted index assigns greater influence
...
\subsubsection{Market Portfolio}\label{market-portfolio}

The market portfolio is the value-weighted return of all stocks in the
sample:
...
month \(t\). Unlike the location and industry indexes, the market
portfolio is not leave-one-out, as the influence of any single firm on
the aggregate market return is negligible.

\subsection{Regression Specification}\label{regression-specification}

For each five-year sub-period, the time-series regression is estimated
separately for each stock. As a result, each firm has its own set of
...
for the five-year period, although all coefficients should be understood
as period-specific.
...
Indexes}\label{model-1-equal-weighted-industry-and-location-indexes}

Recall that Model 1 is the direct replication of the original Pirinsky
...
Figure 1 illustrates the location-index composition for the most
...
{\centering \includegraphics[width=1\linewidth]{second_draft_files/figure-latex/fig-location-panels-1} 

}

\caption{Firm-level empirical location-index weights at the final snapshot month (December 2022) of the 2018--2022 estimation window, under each of the four model/subsample variants in Table 5. Within each panel, firms are ordered from smallest to largest by market capitalization at the snapshot, and bar height reports each firm's absolute weight in the corresponding leave-one-out location index. The y-axis is on a common logarithmic scale across all four panels for direct visual comparison. Each panel's subtitle reports the MSA and number of firms N.}\label{fig:fig-location-panels}
\end{figure}
...
considerably once both the industry and location indexes are
value-weighted, as shown in the next section.

\subsection{Model 2: Value-Weighted Industry and Location
Indexes}\label{model-2-value-weighted-industry-and-location-indexes}
...
statistically significant after both the location and industry indexes
are tilted toward larger firms. Model 2 therefore strengthens the
persistence result: even when the local index itself is rebuilt as a
...
times larger. Second, while the original study reported a declining
location effect over time (0.545 \(\to\) 0.532 \(\to\) 0.459), the
modern broad-sample estimates move in the opposite direction, tracing a
monotone increase (0.200 \(\to\) 0.213 \(\to\) 0.295). The decline
...
The results from the modern sample show that the location effect
...
Model 1 (the equal-weighted replication), \(\hat{\beta}_{Loc}\) rises
from 0.200 (\(t=9.91\)) in 2008--2012 to 0.295 (\(t=13.33\)) in
2018--2022. Under Model 2 (the value-weighted extension), the location
...
S\&P 500 firms, but once the location and industry indexes are
...
variation in stock returns---albeit at roughly half of the magnitude
reported for the 1988--2002 sample in the original paper.
...
statistically indistinguishable from zero under the equal-weighted index
design, yet clearly positive under the value-weighted index design. Two
...
once the location and industry indexes are value-weighted, the
...
small, and that it can only be detected once the index construction no
...
\item
  ...
  and industry indexes are constructed in a size-aware way.
\item
...
In summary, this study replicates and extends Pirinsky and Wang (2006)
...
survives a specification in which both the industry index and the
location index are value-weighted. The digital revolution and the trend
toward remote work have therefore not dissolved the geographic
co-movement effect: physical corporate headquarters location continues
to explain a statistically significant portion of return co-movement in
the modern U.S. market, and that conclusion holds even when the location
and industry indexes are reweighted toward the largest firms. Future
...
\emph{Journal of Financial Economics}, 43(2), 153--193.
...
Center for Research in Security Prices, LLC (2025). \emph{CRSP Monthly
Stock File}. Accessed via Wharton Research Data Services,
\url{https://wrds-www.wharton.upenn.edu/}, November 2025.

French, K. R. (2025). \emph{Detail for 48 Industry Portfolios}. Kenneth
R.~French Data Library,
\url{https://mba.tuck.dartmouth.edu/pages/faculty/ken.french/data_library.html},
accessed November 2025.

Loughran, T., and McDonald, B. (2024). \emph{10-K Header Data,
1993--2024}. Software Repository for Accounting and Finance, University
of Notre Dame, \url{https://sraf.nd.edu/sec-edgar-data/}, accessed
November 2025.

S\&P Global Market Intelligence (2025). \emph{Compustat North America
and CRSP-Compustat Merged Database}. Accessed via Wharton Research Data
Services, \url{https://wrds-www.wharton.upenn.edu/}, November 2025.

U.S. Department of Housing and Urban Development (2023). \emph{HUD USPS
ZIP Code Crosswalk Files (ZIP--CBSA, December 2023 vintage)}. Office of
